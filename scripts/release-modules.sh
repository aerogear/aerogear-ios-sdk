...
do
  ...
  pod lib lint ${module}.podspec  --swift-version=${SWIFT_VERSION} --no-subspecs --no-clean --allow-warnings --verbose
  if [ $? -eq 0 ]; then
...
do
  ...
  pod trunk push ${module}.podspec --skip-import-validation --swift-version=${SWIFT_VERSION} --allow-warnings --verbose
